...
def _extract_sample_series(details: dict) -> list:
    """Down-sample the activity-details time series to one record every 10s,
    extracting heart rate / power / cadence by matching metric descriptor keys."""
    ...
    def find_index(*candidates):
        for c in candidates:
            for d in descriptors:
                if (d.get("key") or "").lower() == c.lower():
                    return d.get("metricsIndex")
        for c in candidates:
            for d in descriptors:
                if c.lower() in (d.get("key") or "").lower():
                    return d.get("metricsIndex")
        return None

    idx_time  = find_index("directElapsedDuration", "sumElapsedDuration", "directTimestamp")
    ...
    series, base_ts, last_stored = [], None, None
    for entry in metrics:
        vals = entry.get("metrics") or []
        if idx_time >= len(vals) or vals[idx_time] is None:
            continue
        raw_t = float(vals[idx_time])
        if raw_t > 1_000_000:  # epoch-ms timestamp rather than elapsed seconds
            if base_ts is None:
                base_ts = raw_t
            elapsed = int(round((raw_t - base_ts) / 1000))
        else:
            elapsed = int(round(raw_t))

        if last_stored is not None and elapsed - last_stored < 10:
            continue
